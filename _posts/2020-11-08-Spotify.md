...
In Python, these are set as local variables that are then passed to Spotipy's prompt_for_user_token() function that generates an authorization token that is then passed to authorization. If the current user is not authorized, the code will prompt the user to login to Spotify, then paste the URL that the browser was redirected to.
